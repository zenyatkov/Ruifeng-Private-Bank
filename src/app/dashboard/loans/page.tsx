...
export default async function LoansPage() {
  ...
  return (
    <div>
      <PageHeader title="Lending" subtitle="Apply for credit facilities." />
      <div className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
        <Panel title="Apply"><LoanForm accounts={userAccounts} /></Panel>
              ...
              {rows.map(loan => (
                <div key={loan.id} className="rounded-2xl border border-ink-900/5 bg-white p-4 hover-lift">
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-3 text-sm md:grid-cols-4">
                    <div><p className="text-xs text-ink-600/50">Principal</p><p className="font-semibold">{formatCurrency(loan.principal, loan.currency)}</p></div>
                    <div><p className="text-xs text-ink-600/50">Outstanding</p><p className="font-semibold">{formatCurrency(loan.outstanding, loan.currency)}</p></div>
                    <div><p className="text-xs text-ink-600/50">Rate</p><p className="font-semibold">{loan.interestRate}%</p></div>
                    <div><p className="text-xs text-ink-600/50">Monthly</p><p className="font-semibold">{loan.monthlyPayment ? formatCurrency(loan.monthlyPayment, loan.currency) : "—"}</p></div>
                  </div>
                  {loan.status === "active" && (
                  ...
                  {loan.status === "paid_off" && <p className="mt-2 text-xs text-jade-600 font-semibold">✓ Fully repaid</p>}
                  {loan.status === "active" && <p className="mt-1 text-xs text-ink-600/60">{loan.startDate ? `Started ${formatDate(loan.startDate)}` : ""}{loan.purpose ? ` · ${loan.purpose}` : ""}</p>}
                </div>
              ))}
            </div>
          )}
